...
def read_simple_cappi(filename, latlon_path):
    ...
    lalon_path: pat to lat/lon matrices

    Returns
    cappis: array
    coords: list of [lon grid, lat grid]
    """
    ...
    bias = [1.0, 3.0, -2.5, -5.5, -4.0, -1.5, 1.0]

    tstamp = datetime.strptime(re.findall(r"\d{12}", filename)[0], "%Y%m%d%H%M%S")
    for i in range(7):
        after = tstamp.date() >= start_date[i].date()
        before = tstamp.date() <= end_date[i].date()
        if after & before:
            cappis[cappis != -99.0] = cappis[cappis != -99.0] + bias[i]
